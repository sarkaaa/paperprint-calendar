...
const canvasExample = (type: CanvasType) => {
  ...
  return (
    <div className='flex-1 text-right' aria-hidden>
...
const CalendarFormField = ({
  ...
  return (
    <div className='flex-1 rounded-md bg-gradient-to-tr from-indigo-50 to-indigo-100 p-4'>
      <h3 className='font-semibold' id={id}>
        {title}
      </h3>
      <div className='my-4 rounded-md bg-indigo-50 p-4' aria-labelledby={id}>
        {formItems.map((item) => (
          <div
            role='group'
            onKeyDown={(e) =>
            ...
            className='item-center my-4 flex cursor-pointer focus:outline-none focus-visible:opacity-100 focus-visible:ring-2 focus-visible:ring-indigo-700 focus-visible:ring-offset-2'
            key={item.id}
            tabIndex={0}
          >
