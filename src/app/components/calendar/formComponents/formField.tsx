...
const canvasExample = (type: CanvasType) => {
  ...
  return (
    <div className='flex-1 text-right'>{canvasTypes[type]?.repeat(5)}</div>
  );
};
          ...
          <div className='my-4 flex cursor-pointer items-center' key={items.id}>
            <div className='relative flex size-5 shrink-0 items-center justify-center rounded-full'>
              <input
                ...
                value={items.value}
                onChange={onChange}
                checked={items.value === calendarValueCheck}
                type='radio'
            ...
            <label
              htmlFor={items.id}
              className={`${items?.example && fontExample(items?.example)} ml-4 cursor-pointer text-base font-semibold`}
            >
              {items.title}
            </label>
